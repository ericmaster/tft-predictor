...
class DataProcessor:
    ...
    def process_single_file(self, file_name: str, verbose: bool = True) -> bool:
        ...
        try:
            ...
            for sample_feature in self.sample_features[1:]:  # Skip heartRate (already in df)
    ...
    def process_all_files(self, max_workers: Optional[int] = None, use_parallel: bool = False) -> Tuple[int, int]:
        """
        ...
        if use_parallel and max_workers:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(self.process_single_file, file_name, verbose=False): file_name 
    ...
    def get_processing_stats(self) -> dict:
        ...
        """
        total_json_files = len([f for f in os.listdir(self.input_dir) if f.endswith('.json')])
        processed_csv_files = len(self.get_processed_files())
        
        return {
            "total_input_files": total_json_files,
            "processed_files": processed_csv_files,
            "remaining_files": total_json_files - processed_csv_files,
...
if __name__ == "__main__":
    ...
    processed_count, skipped_count = data_processor.process_all_files(use_parallel=True, max_workers=4)
